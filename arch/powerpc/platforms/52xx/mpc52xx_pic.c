...
static void mpc52xx_sdma_hold(struct irq_data *d)
{
	int l2irq = irqd_to_hwirq(d) & MPC52xx_IRQ_L2_MASK;
	io_be_setbit(&sdma->IntMask, l2irq);
	out_be32(&sdma->IntPend, 1 << l2irq);
